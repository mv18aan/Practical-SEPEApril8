...
System.out.println("Start time is not same as previous value");
}
...
subClip2.setTitle(SubTitle2); // Set Ttile 
subClip2.setStart(6);//Set start time to sub video from 5th sec 
subClip2.setEnd(50);// set end time to sub video at 40th second

boolean DuplicateClip = subClip1.equals(subClip2); 
assertEquals(false,DuplicateClip); // its a duplicate video
System.out.println("Duplicate video"); 
...
{ 
System.out.println("Test to check Start time is Positive");
Clip instance = new Clip();

int originalStarttime = 1;
instance.setMax(100); 
instance.setStart(originalStarttime);
int StartTime = instance.getStart(); 
instance.setStart(StartTime);
if(originalStarttime>0){
System.out.println("Positive Start value set");
}
}
